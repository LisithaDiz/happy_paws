...
// Debug output
echo "<!-- Debug Info:\n";
echo "vet_id: " . (isset($vet_id) ? $vet_id : 'not set') . "\n";
echo "vet_name: " . (isset($vet_name) ? $vet_name : 'not set') . "\n";
...
<head>
    ...
    <link rel="stylesheet" href="<?= ROOT ?>/assets/css/reviews2.css">
</head>
<body>
    ...
    <div class="container">
        <div class="reviews-container">
            <?php if (isset($is_editing) && isset($review)): ?>
                <!-- Edit Review Form -->
                <h3 class="search-heading">Edit Review for <?= htmlspecialchars($vet_name) ?></h3>
                <div class="edit-review-container vet-card">
                    <form method="POST" action="<?= ROOT ?>/reviews2/edit/<?= $review->review_id ?>">
                        ...
                        <div class="button-group">
                            ...
                            <a href="<?= ROOT ?>/reviews2/index/<?= $review->vet_id ?>" class="button search-button">Cancel</a>
                        </div>
                    </form>
                </div>
            <?php else: ?>
                <h2 class="search-heading">
                    Reviews for <?= isset($vet_name) && $vet_name ? htmlspecialchars($vet_name) : 'Veterinarian' ?>
                    <?php if (isset($error)): ?>
                ...
                <?php elseif (!isset($has_reviewed) || !$has_reviewed): ?>
                    ...
                    <div class="add-review-container vet-card">
                        <?php
                        echo "<!-- Debug: \n";
                        echo "vet_id: " . (isset($vet_id) ? $vet_id : 'not set') . "\n";
                        ...
                        <form method="POST" action="<?= ROOT ?>/reviews2/add" onsubmit="return validateForm()">
                            <?php if(isset($error)): ?>
                                <div class="error-message"><?= htmlspecialchars($error) ?></div>
                            <?php endif; ?>
                            ...
                            <div class="vet-card">
                                ...
                                <?php if (isset($_SESSION['owner_id']) && $_SESSION['owner_id'] == $review->owner_id): ?>
                                    <div class="review-actions">
                                        <a href="<?= ROOT ?>/reviews2/edit/<?= $review->review_id ?>" class="button search-button">Edit</a>
                                        <form method="POST" action="<?= ROOT ?>/reviews2/delete/<?= $review->review_id ?>" style="display: inline;">
    ...
    function validateForm() {
        ...
        var vetId = document.querySelector('input[name="vet_id"]').value;
